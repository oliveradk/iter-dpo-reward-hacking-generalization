...
`pessimistic_training.iterative_training.train`.

The concrete per-backend `train_dpo` / `train_sft` implementations live in
`pessimistic_training.train.train_providers`; they are imported lazily inside
...
CLI (one-off training from a merged standardized JSONL):
    ...
    python -m pessimistic_training.train.train <training_file.jsonl> \\
...
def _dispatch_train_sft(cfg: TrainConfig) -> TrainResult:
    ...
    if cfg.provider == "openai":
        from pessimistic_training.train.train_providers.openai.sft import (
    ...
    if cfg.provider == "modal":
        from pessimistic_training.train.train_providers.modal.sft import (
...
def _dispatch_train_dpo(cfg: TrainConfig) -> TrainResult:
    ...
    modal_swift)."""
    if cfg.provider == "openai":
        from pessimistic_training.train.train_providers.openai.dpo import (
    ...
    if cfg.provider == "tinker":
        from pessimistic_training.train.train_providers.tinker.dpo import (
    ...
    if cfg.provider == "together":
        from pessimistic_training.train.train_providers.together.dpo import (
    ...
    if cfg.provider == "modal":
        from pessimistic_training.train.train_providers.modal.dpo import (
