...
bundles ms-swift + Megatron + transformer-engine):
  MODAL_TRAIN_BASE_MODEL=Qwen/Qwen3-235B-A22B-Instruct-2507 MODAL_TRAIN_GPU=H100:8 \
      modal deploy pessimistic_training/modal/modal_apps/modal_swift_train/modal_sft_swift_app.py
...
self-contained (no `pessimistic_training` imports) and only differs from the DPO
